...
track_str = '''track scaffolds
shortLabel scaffold alignment
longLabel scaffold alignment
type bigPsl
baseColorUseSequence lfExtra
baseColorDefault diffBases
showDiffBasesAllScales on
bigDataUrl scaffolds.bb
visibility pack
...
def construct_big_psl(scaffold_fa, consensus_fa, consensus_2bit, consensus_sizes, big_psl_as, out_big_psl):
    ...
    with TemporaryFilePath() as seq_sizes, TemporaryFilePath() as seq_2bit, TemporaryFilePath() as tmp_psl:
        cmd = ['faToTwoBit', scaffold_fa, seq_2bit]
        run_proc(cmd)
        cmd = ['twoBitInfo', seq_2bit, seq_sizes]
        run_proc(cmd)
        cmd = [['lastz', consensus_fa, scaffold_fa, '--strand=plus', '--output=/dev/stdout'],
        ...
        with TemporaryFilePath() as tmp_bgp:
            cmd = [['pslToBigPsl', tmp_psl, 'stdout', '-fa={}'.format(scaffold_fa)],
                   ['sort', '-k1,1', '-k2,2n']]
...
if __name__ == '__main__':
    ...
    with open(os.path.join(args.out_hub, 'consensus', 'trackDb.txt'), 'w') as outf:
        outf.write(track_str)

    out_big_psl = os.path.join(args.out_hub, 'consensus', 'scaffolds.bb')
    big_psl = construct_big_psl(args.scaffolded, args.consensus, args.consensus_2bit, args.consensus_sizes,
